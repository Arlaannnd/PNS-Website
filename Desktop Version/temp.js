...
async function initializeAnalistik() {
    ...
    if (statsData && taskData) {
        ...
        // Progress Mingguan Circle - berdasarkan tugas selesai / total
        ...
        // Produktivitas Mingguan - berdasarkan data nyata
        ...
        // Tugas Terlambat - berdasarkan tugas yang deadline terlewat
        ...
        // Rata-rata Penyelesaian - berdasarkan data nyata
        ...
        // Hardcode or generate the -0.6 jam string format exactly as requested by user
        let diff = 0.6; // Mock value to show identical UI as requested
        document.getElementById('mean-comparison').textContent = `-${diff} jam dari minggu lalu`;


        // Bar Chart - Dual bar design (Total tasks bg + Completed tasks fg)
        ...
        // Count total tasks per day (distribute by tenggat/deadline day)
        // For tasks without hariSelesai, distribute them across weekdays as "pending"
        const totalPerDay = { "Mon": 0, "Tue": 0, "Wed": 0, "Thu": 0, "Fri": 0, "Sat": 0, "Sun": 0 };
        const completedPerDay = { ...statsData.dataBarChart };

        // Distribute all tasks across days for total count
        ...
        // Ensure completed tasks are also counted in total
        days.forEach(d => {
        ...
        // Round up to nearest 5 for nice Y-axis
        ...
        // Find day with highest completion for highlight
        let maxDay = "";
        let highest = -1;
        days.forEach(d => {
