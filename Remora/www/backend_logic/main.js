...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Tambahkan ini di dalam blok document.addEventListener('DOMContentLoaded', ...)

    window.authService.getSupabase().then(supabase => {
        // Dengarkan perubahan status otentikasi secara real-time
        supabase.auth.onAuthStateChange((event, session) => {
            const loader = document.getElementById('global-loader');

            if (event === 'SIGNED_IN') {
                // Jika login berhasil, langsung sembunyikan loader dan masuk ke dashboard
                if (loader) loader.style.display = 'none';
                showView(views.main); 
                showContentView('dashboard-content');
                loadDataAsync();
            } else if (event === 'SIGNED_OUT' || !session) {
                // Jika tidak ada sesi (belum login), sembunyikan loader dan tampilkan halaman awal
                if (loader) loader.style.display = 'none';
